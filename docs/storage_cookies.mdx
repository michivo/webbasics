...
- `Partitioned`: Cookies mit diesem Attribut werden verwendet, um bei eingebetteten Inhalten von Drittanbietern (wie z.B. `iframe`s) Cookies auch dann unterstützen zu können, wenn 3rd-Party-Cookies eigentlich blockiert werden. Wird z.B. ein solches Cookie für in die Seite example.com eingebettete Inhalte der Domain external.com gesetzt, dann wird es auch nur beim Laden von Inhalten von external.com übertragen, wenn diese Inhalte in eine Seite von example.com eingebettet sind. Sind die Inhalte von external.com in die Seite alternative.com eingebettet, werden diese Cookies *nicht* an external.com übertragen. Somit wird Tracking verunmöglicht, Authentifzierung beim Laden externer Inhalte aber ermöglicht.
